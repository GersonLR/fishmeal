...
function [XLOADINGS, YLOADINGS, XSCORES, YSCORES, coefficients, fitted,projection] = plsregressGLR (X, Y, NCOMP)
  ...
  Xmeans = mean (X);
  Xstd=std (X);
  X = bsxfun (@rdivide, bsxfun (@minus,X, Xmeans), Xstd);

  ## Mean centering and rescaled Data Y
  Ymeans = mean (Y);
  Ystd=std (Y);
  Y = bsxfun (@rdivide, bsxfun (@minus, Y, Ymeans), Ystd);
  ...
  fitted = bsxfun (@plus,bsxfun (@times, T*Q', Ystd),Ymeans); # Add mean and std
